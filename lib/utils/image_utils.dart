...
class ImageUtils {
  ...
  static Future<String> saveImage(File imageFile, {String? filename}) async {
    ...
    final name = filename ?? p.basename(imageFile.path);
    final newPath = p.join(imagesDir.path, name);
    return await imageFile.copy(newPath).then((f) => f.path);
  }
  ...
  static Future<String> generateThumbnail(
    ...
    final name = filename ?? "thumb_${p.basename(originalFile.path)}";
    final newPath = p.join(thumbsDir.path, name);
